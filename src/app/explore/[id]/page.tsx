...
export default function PetDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Find current pet
  const pet = useMemo(() => {
    return placeholderPets.find((p) => p.id === petId);
  }, [petId]);

  // Related pets: same species, excluding current pet
  const relatedPets = useMemo(() => {
    if (!pet) return [];
    return placeholderPets
      .filter((p) => p.species.toLowerCase() === pet.species.toLowerCase() && p.id !== pet.id)
      .slice(0, 4); // Limit to 4 related pets
  }, [pet]);
  ...
  const [aiChatHistory, setAiChatHistory] = useState([
    {
      role: "assistant",
      text: `Hi! I'm Paws, your AI adoption guide. Ask me anything about ${pet?.name || "this pet"} (e.g., "Is they good with kids?", "What is their energy level?")!`
    }
  ]);
      ...
      <div className="flex flex-col min-h-screen bg-slate-50">
        ...
        <main className="flex-1 flex flex-col items-center justify-center p-8">
          <div className="bg-white p-8 rounded-3xl border border-slate-200/60 shadow-sm text-center max-w-md">
            ...
            <p className="text-slate-650 mb-6">The pet you are looking for might have been adopted or the link is invalid.</p>
  ...
  const handleAiMessageSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      let reply = "";
      if (currentQuestion.includes("kid") || currentQuestion.includes("child")) {
      ...
      } else if (currentQuestion.includes("pet") || currentQuestion.includes("dog") || currentQuestion.includes("cat")) {
      ...
      } else if (currentQuestion.includes("vaccin") || currentQuestion.includes("shot")) {
      ...
      } else if (currentQuestion.includes("neuter") || currentQuestion.includes("spay")) {
        reply = pet.neutered
      ...
      } else if (currentQuestion.includes("fee") || currentQuestion.includes("cost")) {
          ...
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-3xl border border-slate-200/60 p-6 md:p-8 shadow-sm space-y-6">
              ...
              <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 pb-6">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-bold text-teal-600 uppercase tracking-widest">{pet.breed}</span>
                  </div>
                  <h1 className="text-3xl md:text-4xl font-black text-slate-800">{pet.name}</h1>
                </div>
                <div>
                  <span className="text-xs font-bold text-slate-400 uppercase block tracking-wider">Adoption Fee</span>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 text-center">
                  <span className="text-xs font-bold text-slate-400 uppercase tracking-wider block mb-1">Gender</span>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 pt-2">
                <div className="flex items-center gap-2.5 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${pet.vaccinated ? 'bg-emerald-500' : 'bg-amber-400'}`}></span>
                  <span className="text-slate-650 font-medium">{pet.vaccinated ? 'Vaccinated' : 'Needs Vaccines'}</span>
                </div>
                <div className="flex items-center gap-2.5 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${pet.neutered ? 'bg-emerald-500' : 'bg-amber-400'}`}></span>
                  <span className="text-slate-650 font-medium">{pet.neutered ? 'Neutered' : 'Not Neutered'}</span>
                </div>
                <div className="flex items-center gap-2.5 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${pet.goodWithKids ? 'bg-emerald-500' : 'bg-red-400'}`}></span>
                  <span className="text-slate-650 font-medium">{pet.goodWithKids ? 'Good with Kids' : 'No Small Kids'}</span>
                </div>
                <div className="flex items-center gap-2.5 p-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm">
                  <span className={`w-2.5 h-2.5 rounded-full shrink-0 ${pet.goodWithPets ? 'bg-emerald-500' : 'bg-red-400'}`}></span>
                  <span className="text-slate-650 font-medium">{pet.goodWithPets ? 'Good with Pets' : 'Single Pet'}</span>
                </div>
              </div>

              {/* Overview / Story Section */}
              <div className="space-y-3 pt-2">
                <h2 className="text-xl font-bold text-slate-800">About {pet.name}</h2>
              ...
              {/* Temperament Tags */}
              <div className="space-y-3 pt-2">
                <h2 className="text-xl font-bold text-slate-800">Temperament</h2>
              ...
              <div className="bg-gradient-to-br from-teal-50 to-emerald-50/50 rounded-2xl border border-teal-100 p-5 mt-6 space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-teal-900 text-sm md:text-base">Ask Paws about {pet.name}</h3>
          ...
          <div className="lg:col-span-4 sticky top-24">
            <div className="bg-white rounded-3xl border border-slate-200/60 p-6 md:p-8 shadow-sm text-center space-y-6">
              ...
              <div className="space-y-1">
                <span className="text-xs font-bold text-slate-400 uppercase tracking-widest block">Available At</span>
                <span className="text-xl font-bold text-slate-800">{pet.location}</span>
              </div>

              <div className="border-t border-slate-100 pt-6">
                <p className="text-slate-500 text-sm leading-relaxed mb-6">
                  Ready to welcome {pet.name} into your home? Start the secure matching and adoption inquiry process today.
                </p>
        ...
        {relatedPets.length > 0 && (
          <section className="border-t border-slate-200/60 pt-16">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              <div>
                <h2 className="text-2xl md:text-3xl font-black text-slate-800">Similar Pets Nearby</h2>
                <p className="text-slate-500 text-sm mt-1">Explore other adorable {pet.species.toLowerCase()}s looking for a family.</p>
              </div>
              <Link href="/explore" className="text-sm font-bold text-teal-700 hover:text-teal-850 transition-colors">
                View All Pets &rarr;
              </Link>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden border border-slate-100 shadow-2xl relative animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-teal-700 text-white px-6 py-5 flex items-center justify-between">
              <div>
                ...
                <p className="text-teal-150 text-xs">Start your adoption application</p>
              </div>
              <button
                onClick={() => {
              ...
              {isApplicationSubmitted ? (
                <div className="text-center py-6 space-y-4">
                  ...
                  <p className="text-slate-650 text-sm">
                    Thank you for applying to adopt {pet.name}! Our shelter coordinators will review your matching criteria and contact you within 24-48 hours.
                  </p>
                  <button
                    onClick={() => {
                      setIsAdoptModalOpen(false);
                ...
                <form onSubmit={handleAdoptSubmit} className="space-y-4">
                  ...
                  {/* Name */}
                  <div className="space-y-1">
                    <label htmlFor="modal-name" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Your Full Name</label>
                    <input
                      id="modal-name"
                      type="text"
                      required
                  ...
                  {/* Email */}
                  <div className="space-y-1">
                    <label htmlFor="modal-email" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Email Address</label>
                    <input
                      id="modal-email"
                      type="email"
                      required
                  ...
                  {/* Message */}
                  <div className="space-y-1">
                    <label htmlFor="modal-msg" className="text-xs font-bold text-slate-500 uppercase tracking-wider">Why are you a good match?</label>
                    <textarea
                      id="modal-msg"
                      rows={4}
