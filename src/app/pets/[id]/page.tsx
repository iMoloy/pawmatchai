...
export default function PetDetailsPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 return relatedData.pets.filter((p: any) => p.id !== pet.id).slice(0, 4);
 }, [relatedData, pet]);
 ...
 {(petError as any)?.response?.status === 404
 ...
 <h1 className="text-fluid-heading mb-6 text-slate-800">
 {pet.name}
 </h1>
 </div>
 <div>
