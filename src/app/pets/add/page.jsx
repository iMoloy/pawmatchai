...
export default function AddPetPage() {
  ...
  const validate = () => {
    ...
    if (!formData.age.trim()) newErrors.age = "Age is required (e.g. '2 years')";
    if (!formData.location.trim()) newErrors.location = "Location is required";
    if (!formData.fee.trim()) newErrors.fee = "Adoption fee is required";
    if (!formData.shortDescription.trim()) newErrors.shortDescription = "Short description is required";
    if (formData.shortDescription.length > 100) newErrors.shortDescription = "Keep it under 100 characters";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ submit: "Failed to add pet. Please try again or check backend server." });
    ...
    <ProtectedRoute>
      <div className="flex flex-col min-h-screen bg-slate-50">
        ...
        <main className="flex-1 py-10 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto w-full">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-black text-slate-800 tracking-tight">Add a Pet</h1>
            <p className="text-slate-500 mt-2">Create a new listing to help a furry friend find their forever home.</p>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
            <div className="p-6 sm:p-10">
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                ...
                {/* Basic Info Section */}
                <div>
                  <h3 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-2 mb-4">Basic Information</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Pet Name</label>
                      <input type="text" name="name" value={formData.name} onChange={handleChange} placeholder="e.g. Luna"
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Species</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Breed</label>
                      <input type="text" name="breed" value={formData.breed} onChange={handleChange} placeholder="e.g. Golden Retriever"
                      ...
                      {errors.breed && <p className="text-xs text-red-500 font-medium mt-1">{errors.breed}</p>}
                    </div>

                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Age</label>
                      <input type="text" name="age" value={formData.age} onChange={handleChange} placeholder="e.g. 2 years, 3 months"
                      ...
                      {errors.age && <p className="text-xs text-red-500 font-medium mt-1">{errors.age}</p>}
                    </div>
                    
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Size</label>
                      <select name="size" value={formData.size} onChange={handleChange}
                        ...
                        <option value="Extra Large">Extra Large (101+ lbs)</option>
                      </select>
                    </div>
                ...
                {/* Details Section */}
                <div>
                  <h3 className="text-lg font-bold text-slate-800 border-b border-slate-100 pb-2 mb-4">Adoption Details</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Location (City, State)</label>
                    ...
                    <div className="space-y-1">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Adoption Fee ($)</label>
                      <input type="text" name="fee" value={formData.fee} onChange={handleChange} placeholder="e.g. 150"
                      ...
                      {errors.fee && <p className="text-xs text-red-500 font-medium mt-1">{errors.fee}</p>}
                    </div>
                    ...
                    <div className="space-y-1 sm:col-span-2">
                      ...
                      <input type="url" name="imageUrl" value={formData.imageUrl} onChange={handleChange} placeholder="https://example.com/image.jpg"
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all text-sm" />
                      <p className="text-xs text-slate-400 mt-1">Leave blank to use a default placeholder image.</p>
                    </div>

                    <div className="space-y-1 sm:col-span-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Short Description</label>
                      <input type="text" name="shortDescription" value={formData.shortDescription} onChange={handleChange} placeholder="A brief, catchy intro for this pet..."
                      ...
                      <div className="flex justify-between items-center mt-1">
                        {errors.shortDescription ? <p className="text-xs text-red-500 font-medium">{errors.shortDescription}</p> : <div></div>}
                        <p className={`text-xs ${formData.shortDescription.length > 100 ? 'text-red-500' : 'text-slate-400'}`}>{formData.shortDescription.length}/100</p>
                      </div>
                    </div>

                    <div className="space-y-1 sm:col-span-2">
                      <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block">Full Description (Optional)</label>
                      <textarea name="fullDescription" value={formData.fullDescription} onChange={handleChange} placeholder="Tell us everything about this pet's personality, history, and needs..." rows={4}
                        className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:bg-white transition-all text-sm resize-none"></textarea>
                    </div>
                  </div>
                </div>

                <div className="pt-4 flex justify-end gap-3">
                  <button type="button" onClick={() => router.back()} className="px-6 py-3.5 bg-slate-100 text-slate-700 font-bold rounded-xl hover:bg-slate-200 transition-colors shadow-sm">
                    Cancel
                  </button>
                  <button
                    type="submit"
